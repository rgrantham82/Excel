...
FROM   country_stats;SELECT   region,
...
GROUP BY region,
       ...
       (
                ...
                ON       s.country_id = c.id
                GROUP BY region,
                         athlete_name ) AS subquerywhere row_num = 1;

SELECT region,
       country,
       Sum(gdp)                         AS country_gdp,
       Sum(Sum(gdp)) OVER ()            AS global_gdp,
       Sum(gdp) / Sum(Sum(gdp)) OVER () AS perc_global_gdpfrom country_stats AS cs
JOIN   countries                        AS c
ON     cs.country_id = c.idwhere gdp IS NOT nullgroup BY region,
       countryorder BY country_gdp DESC;` `SELECT region,
       country,
       Sum(gdp)                                            AS country_gdp,
       Sum(Sum(gdp)) OVER ()                               AS global_gdp,
       Sum(gdp) / Sum(Sum(gdp)) OVER ()                    AS perc_global_gdp,
       Sum(gdp) / Sum(Sum(gdp)) OVER (partition BY region) AS perc_region_gdpfrom country_stats AS cs
JOIN   countries                                           AS c
ON     cs.country_id = c.idwhere gdp IS NOT nullgroup BY region,
       countryorder BY country_gdp DESC;
SELECT region,
       country,
       Sum(gdp)              / Sum(pop_in_millions)                                                    AS gdp_per_million,
       Sum(Sum(gdp)) OVER () / Sum(Sum(pop_in_millions)) OVER ()                                       AS gdp_per_million_total,
       (Sum(gdp) / Sum(pop_in_millions)) / (Sum(Sum(gdp)) OVER () / Sum(Sum(pop_in_millions)) OVER ()) AS performance_indexfrom country_stats_clean AS cs
JOIN   countries                                                                                       AS c
ON     cs.country_id = c.idwhere year = '2016-01-01'
AND    gdp IS NOT nullgroup BY region,
       countryorder BY gdp_per_million DESC;
